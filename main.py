...
class WeatherApp:
    ...
    def get_weather(self):
        ...
        print(f"informacje:{self.forecast_weather_data}")
        ...
        self.plot_temperature_chart(current_date)
    ...
    def display_current_weather(self):
        ...
        self.city_name_var.set(f"{self.current_weather_data['city_name']}")
        self.timestamp_var.set(f"Aktualna pogoda\n{self.current_weather_data['timestamp']}")
        print(f"seimano:{self.current_weather_data['timestamp']}")
        self.description_var.set(f"{self.current_weather_data['description']}")
        self.temperature_var.set(f"{self.current_weather_data['temperature']}°C")
        self.feels_like_var.set(f"Temp. odczuwalna {self.current_weather_data['feels_like']}°C")
        self.high_var.set(f"Najwyższa temp.\n{self.current_weather_data['high']}°C")
        self.low_var.set(f"Najniższa temp.\n{self.current_weather_data['low']}°C")
        self.humidity_var.set(f"Wilgotność\n{self.current_weather_data['humidity']}%")
        self.wind_speed_var.set(f"Wiatr\n{self.current_weather_data['wind_speed']} m/s")
        self.cloudiness_var.set(f"Zachmurzenie\n{self.current_weather_data['cloudiness']}%")
        self.rain_var.set(f"Opady deszczu (1h)\n{self.current_weather_data['rain']} mm")
        sunrise_time = datetime.strptime(self.current_weather_data['sunrise'], '%H:%M %A %d-%m-%Y').strftime('%H:%M')
        self.sunrise_var.set(f"Wschód\n{sunrise_time}")
        sunset_time = datetime.strptime(self.current_weather_data['sunset'], '%H:%M %A %d-%m-%Y').strftime('%H:%M')
    ...
    def display_5_day_forecast(self):
        ...
        for i, date in enumerate(daily_temps.keys()):
            ...
            button = ttk.Button(forecast_frame, text="Pokaż wykres", command=lambda d=date: self.display_temperature_chart_for_day(d))
    ...
    def load_last_location(self):
        ...
        with open("last_location.txt", "r") as file:
            ...
            self.button = ttk.Button(self.root, text="Search", command=self.get_weather)
    ...
    def create_widgets(self):
        ...
        self.plot_temperature_chart(datetime.now().strftime('%A'))

    def plot_temperature_chart(self, date):
        try:
            hours = list(self.forecast_weather_data[datetime.now().strftime('%A')].keys())
            print(hours)

            hour_objects = [datetime.strptime(hour, '%H:%M') for hour in hours]

            temperatures = [data['temperature'] for data in self.forecast_weather_data[datetime.now().strftime('%A')].values()]
            print(temperatures)
    ...
    def display_temperature_chart_for_day(self, day):
        try:
            hours = list(self.forecast_weather_data[day].keys())

            hour_objects = [datetime.strptime(hour, '%H:%M') for hour in hours]

            temperatures = [data['temperature'] for data in self.forecast_weather_data[day].values()]
